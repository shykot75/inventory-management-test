...
@section('breadcrumbs')
    <x-breadcrumb.items>
        <x-slot:title>Sale Lists</x-slot:title>
        <x-breadcrumb.item route="{{ route('admin.dashboard') }}" postion="first"> Dashboard </x-breadcrumb.item>
        <x-breadcrumb.item route="{{ route('admin.sale.create') }}" position="middle">Add New Sale</x-breadcrumb.item>
        <x-breadcrumb.item position="last" type="active">Sale Lists</x-breadcrumb.item>
    </x-breadcrumb.items>
@endsection
...
@section('content')
    <div class="main-body">
        <div class="card">
            <div class="card-body">
                <h6 class="mb-4 text-15">Sale Lists</h6>
                <table id="withExportBtnDatatable" class="bordered group" style="width:100%">
                    ...
                    @foreach($sales as $item)
                        <tr class="text-sm text-center">
                            ...
                            <td>{{ $item->sale_total ?? '--' }} Tk</td>
                            <td>
                                <div class="{{ $item->payment_status == \App\Enums\SalesEnum::PAYMENT_STATUS_PAID ?'badge-active': 'badge-inactive' }}  px-2.5 py-0.5 inline-block">
                                    {{ $item->payment_status == \App\Enums\SalesEnum::PAYMENT_STATUS_PAID ? 'Paid' : 'Not Paid'}}
                                </div>
                            </td>
                            <td>
                                            ...
                                            <ul class="absolute z-50 py-2 mt-1 list-none bg-white rounded-md shadow-md ltr:text-left rtl:text-right dropdown-toggle-menu min-w-max dark:bg-zinc-600 mr-4"
                                                ...
                                                <li>
                                                    <a class="block px-4 py-1.5 text-base font-medium transition-all duration-200 ease-linear text-slate-600 dropdown-item hover:bg-slate-100 hover:text-slate-500 focus:bg-slate-100 focus:text-slate-500 dark:text-zinc-100 dark:hover:bg-zinc-500 dark:hover:text-zinc-200 dark:focus:bg-zinc-500 dark:focus:text-zinc-200"
                                                       href="{{ route('admin.sale.return.create', $item->sale_id) }}">
                                                        <i class="fa-solid fa-pen-to-square mr-1"></i> Return</a>

                                                </li>
                                                <li>
                                                    <a class="block px-4 py-1.5 text-base font-medium transition-all duration-200 ease-linear text-slate-600 dropdown-item hover:bg-slate-100 hover:text-slate-500 focus:bg-slate-100 focus:text-slate-500 dark:text-zinc-100 dark:hover:bg-zinc-500 dark:hover:text-zinc-200 dark:focus:bg-zinc-500 dark:focus:text-zinc-200"
                                                       href="{{ route('admin.sale.pdf', $item->sale_id) }}">
