...
@section('title')
     Purchase Return Lists | Admin
@endsection

@section('breadcrumbs')
    <x-breadcrumb.items>
        <x-slot:title>Purchase Return Lists</x-slot:title>
        <x-breadcrumb.item route="{{ route('admin.dashboard') }}" postion="first"> Dashboard </x-breadcrumb.item>
        <x-breadcrumb.item position="last" type="active">Purchase Return Lists</x-breadcrumb.item>
    </x-breadcrumb.items>
@endsection
                ...
                <table id="withExportBtnDatatable" class="bordered group" style="width:100%">
                    ...
                    @foreach($purchaseReturns as $item)
                        <tr class="text-sm text-center">
                            ...
                            <td>
                                <div class="{{ $item->payment_status == \App\Enums\PurchaseReturnEnum::PAYMENT_STATUS_PAID ?'badge-active': 'badge-inactive' }}  px-2.5 py-0.5 inline-block">
                                    {{ $item->payment_status == \App\Enums\PurchaseReturnEnum::PAYMENT_STATUS_PAID ? 'Paid' : 'Not Paid'}}
                                </div>
                            </td>
                                            ...
                                            <ul class="absolute z-50 py-2 mt-1 list-none bg-white rounded-md shadow-md ltr:text-left rtl:text-right dropdown-toggle-menu min-w-max dark:bg-zinc-600"
                                                ...
                                                <li>
                                                    <a class="block px-4 py-1.5 text-base font-medium transition-all duration-200 ease-linear text-slate-600 dropdown-item hover:bg-slate-100 hover:text-slate-500 focus:bg-slate-100 focus:text-slate-500 dark:text-zinc-100 dark:hover:bg-zinc-500 dark:hover:text-zinc-200 dark:focus:bg-zinc-500 dark:focus:text-zinc-200"
                                                       href="{{ route('admin.report.purchase.return.pdf', $item->purchase_return_id  ?? '') }}">
