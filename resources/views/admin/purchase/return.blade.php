...
@section('breadcrumbs')
    <x-breadcrumb.items>
        <x-slot:title>New Purchase Return</x-slot:title>
        <x-breadcrumb.item route="{{ route('admin.dashboard') }}" position="first"> Dashboard </x-breadcrumb.item>
        <x-breadcrumb.item route="" position="middle"> Purchase Return List </x-breadcrumb.item>
        <x-breadcrumb.item position="last" type="active">Create Purchase Return</x-breadcrumb.item>
    </x-breadcrumb.items>
@endsection
...
@section('content')
    <div class="main-body">
        <div class="card">
            ...
            <div class="card-body">
                <form action="{{ route('admin.purchase.return.store' ) }}" method="POST">
                    @csrf
                    ...
                    <div class="grid xl:grid-cols-3 gap-x-4 my-2">
                        ...
                        <div>
                            ...
                            <input type="text" id="product_id" class="form-input "
                                   value="{{ $purchase->product->product_name }} (Tk-{{ $purchase->product->product_price }}) CS:{{ $purchase->product->quantity }}" readonly >
                        </div>
                        <!-- Product -->
                        ...
                        <div>
                            ...
                            <input type="number" name="return_quantity" id="return_quantity" class="form-input" min="1" value="{{ $purchase->purchase_quantity }}" required readonly>
                        </div>
                    </div>
                    <div class="grid xl:grid-cols-3 gap-x-4 my-2">
                        <!-- Return Amount -->
                        <div>
                            ...
                            <input type="text" name="return_amount" id="return_amount" class="form-input" value="{{ $purchase->purchase_total }}" required readonly>
                        </div>
                        ...
                        <div>
                            ...
                            <select name="payment_type" class="form-input form-select" id="payment_type" required>
                                ...
                                @foreach(\App\Enums\PurchaseReturnEnum::PAYMENT_TYPE_LABELS as $label => $value)
                        ...
                        <div>
                            ...
                            <select name="payment_status" class="form-input form-select" id="payment_status" required>
                                <option selected disabled>--select payment status--</option>
                                @foreach(\App\Enums\PurchaseReturnEnum::PAYMENT_STATUS_LABELS as $label => $value)
                    ...
                    <div class="w-full">
                        <button type="submit" class="btn bg-primary-500 text-white">Create Purchase Return</button>
                    </div>
                </form>
            </div>
        </div>
